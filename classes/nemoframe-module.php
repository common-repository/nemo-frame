	...
	if(function_exists('get_called_class')) {
		abstract class NEMOFRAME_Module {
			...
			public function __get( $variable ) {
				...
				eval("\$arrays=".$module."::\$readable_properties;");
				if ( in_array( $variable, $arrays ) ) {
			...
			public function __set( $variable, $value ) {
				...
				eval("\$arrays=".$module."::\$writeable_properties;");
				if ( in_array( $variable, $arrays ) ) {
					$this->$variable = $value;
			...
			protected static function render_template( $default_template_path = false, $variables = array(), $require = 'once' ) {
				do_action( 'nemoframe_render_template_pre', $default_template_path, $variables );
				...
				$template_path = apply_filters( 'nemoframe_template_path', $template_path );
				...
				if ( is_file( $template_path ) ) {
					...
					$template_content = apply_filters( 'nemoframe_template_content', ob_get_clean(), $default_template_path, $template_path, $variables );
				} else {
					$template_content = '';
				}
	
				do_action( 'nemoframe_render_template_post', $default_template_path, $variables, $template_path, $template_content );
		...
		} // end NEMOFRAME_Module
	} else {
		abstract class NEMOFRAME_Module {
			private static function get_called_class($level = 1, $trace = false) {
